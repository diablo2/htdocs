...
$schema_changes = array(
	'add_columns' => array(
		USERS_TABLE => array(
			'user_congrats' => array('BOOL', 0),
		),
	),
);
$sql_array = array_merge($sql_array, $db_tools->perform_schema_changes($schema_changes));
$schema_changes = array(
	'change_columns' => array(
		USERS_TABLE => array(
			'user_show_age' => array('BOOL', 0),
		),
	),
);
$sql_array = array_merge($sql_array, $db_tools->perform_schema_changes($schema_changes));

$sql = 'SELECT user_id, user_show_age'
		. ' FROM ' . USERS_TABLE
		. ' WHERE user_show_age > 0';
$result = $db->sql_query($sql);
$row = array();
while ($row = $db->sql_fetchrow($result))
{
	$update_info['user_show_age'] = ($row['user_show_age'] & 0x01);
	$update_info['user_congrats'] = ($row['user_show_age'] & 0x02) >> 1;
	$sql_array[] = 'UPDATE ' . USERS_TABLE
					. ' SET ' . $db->sql_build_array('UPDATE', $update_info)
