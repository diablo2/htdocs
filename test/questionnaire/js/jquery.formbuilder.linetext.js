/*
 * JQuery Form Builder - Line Text plugin.
 * 
 * Revision: @REVISION
 * Version: @VERSION
...
var FbLineText = $.extend({}, $.fb.fbWidget.prototype, {
	...
	_getWidget : function(event, fb) {
		...
		fb.target._log('fbLineText._getWidget executing...');
		...
		fb.target._log('fbLineText._getWidget executed.');
		return $jqueryObject;
	},
	_getFieldSettingsLanguageSection : function(event, fb) {
		fb.target._log('fbLineText._getFieldSettingsLanguageSection executing...');
    ...
    var fontFamily = styles.fontFamily != 'default' ? styles.fontFamily : fbStyles.fontFamily ;
	  var fontSize = styles.fontSize != 'default' ? styles.fontSize : fbStyles.fontSize;	  
		...
		fb.target._log('fbLineText._getFieldSettingsLanguageSection executed.');
	...
	_getFieldSettingsGeneralSection : function(event, fb) {
		fb.target._log('fbLineText._getFieldSettingsGeneralSection executing...');
		...
		fb.target._log('fbLineText._getFieldSettingsGeneralSection executed.');
		return [$valuePanel, $colorPanel];
	}, 
	_languageChange : function(event, fb) {
		fb.target._log('fbLineText.languageChange executing...');
		var styles = fb.settings.styles;
		var fbStyles = fb.target._getFbLocalizedSettings().styles;
		var fontFamily = styles.fontFamily != 'default' ? styles.fontFamily : fbStyles.fontFamily;
		var fontSize = styles.fontSize != 'default' ? styles.fontSize : fbStyles.fontSize;
    ...
    fb.item.css('fontFamily', fontFamily);
		...
		fb.target._log('fbLineText.languageChange executed.');
	}
});

$.widget('fb.fbLineText', FbLineText);
