          ...
          <div class="row-fluid"> <!-- Contain Search-->
            <div class="span12" >
              <table class="table table-hover demo" id="pagination">
                <thead>
                  <tr>
                    <th class="span1 sortable-numeric fd-column-0">#</th>
                    <th class="span8 sortable-text fd-column-1">Topic</th>
                    <th class="span2 sortable-date-ymd fd-column-2">Date</th>
                    <th class="span1 sortable-text fd-column-3">Done</th>
                  </tr>
                </thead>
              ...
              $where = "";
                if (!empty($_GET)){ // check $_GET
                  $where = 'WHERE questionnaire.name LIKE "%'.$_GET['search'].'%" ';
                } 
                include ('action/show.php');
                  $result_pr = mysql_query($sql) or die(mysql_error());
                        ...
                        for ($j = 1 ; $j <= $page_row ; $j++ ){
                          if ($row_pr = mysql_fetch_array($result_pr)) { // check Query have data
                            ...
                            echo  '<tr>
                              ...
                              <td clas="sized1"><a href="view2.php?key='.base64_encode($row_pr['id']).'">
                                '.$row_pr['name'].'
                                </a>
                              </td>
                              <td class="sized2">
                              ...
                              <td>
                                <label style="display:none">'.$row_pr['count'].'</label><i class="'.$class .'"></i>
                              </td>
                              </tr>';
                          } else {
                            break;
